...
{
//  Commented out because somehow this was logging people out.
//   (and we're not using time dropdowns anyway) Daniel 8/1/08
//	if(MSIE){
...
function change_account_type(type) {
  $$('.show_loc, .show_ib').each(function (e) {
...
function change_debt(id) {
  $$('#'+id+'-principal, #'+id+'-starts-on, #'+id+'-term, #'+id+'-canadian, #'+id+'-interest-only-expires-on').each(function (e) { e.hide(); });
  ...
  if (($F(id+'-kind') == 'mortgage') || ($(id+'-io').checked)) {
    $$('#'+id+'-principal, #'+id+'-starts-on, #'+id+'-term').each(function (e) { e.show(); });
  }
}
